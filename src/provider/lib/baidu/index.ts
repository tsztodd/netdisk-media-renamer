...
export default class ProviderBaidu extends Provider {
  ...
  async getOriginList() {
    ...
    // 无法识别时返回空数组 → 基类回退为默认全选。
    try {
      const selected = vue?.selectedList;
      this._selectedIds = Array.isArray(selected)
        ? selected.map((it: any) => String(it.fs_id)).filter((id: string) => id && id !== "undefined")
        : [];
    } catch {
      this._selectedIds = [];
    }
