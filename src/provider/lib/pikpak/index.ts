...
export default class ProviderQuark extends Provider {
  ...
  async getOriginList() {
    ...
    // 读取网盘当前已勾选的文件（best-effort，需在真实环境核实属性名）。
    try {
      const fileState: any = vue?.config?.globalProperties?.$pinia?.state?.value?.file;
      const selRaw =
        fileState?.selectedFiles || fileState?.checkedFiles || fileState?.selectedIds || [];
      let selectedIds: string[] = Array.isArray(selRaw)
        ? selRaw.map((it: any) => String(typeof it === "object" && it ? it.id : it))
        : [];
      if (!selectedIds.length) {
        selectedIds = originList
          .filter((it: any) => it.kind === "drive#file" && (it._selected || it.selected || it.checked))
