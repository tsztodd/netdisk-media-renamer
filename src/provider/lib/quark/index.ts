...
export default class ProviderQuark extends Provider {
  ...
  async getOriginList() {
    ...
    try {
      const fileState: any = state.file[state.file.listType];
      const selRaw =
        fileState?.selectedFids || fileState?.selectedRowKeys || fileState?.checkedKeys || [];
      let selectedIds: string[] = Array.isArray(selRaw) ? selRaw.map(String) : [];
      if (!selectedIds.length) {
        selectedIds = originList
          .filter((it: any) => it.file && (it._select || it.selected || it.checked))
          .map((it: any) => String(it.fid));
      }
